...
<head>
    ...
    <title>Pro Calc Stable</title>
    <link rel="manifest" href="manifest.json">
<meta name="theme-color" content="#ff9500">
<link rel="apple-touch-icon" href="https://ui-avatars.com/api/?name=T&background=ff9500&color=fff&size=192">
...
<script>
  if ('serviceWorker' in navigator) {
    ...
    margin-bottom: 8px; 
    min-height: 30px; 
    max-height: 25vh; /* Maximum height taake screen ko dhakka na lagay */
    overflow-y: auto; /* Agar boht zyada lines hon to scroll ho jaye */
    word-wrap: break-word; /* Bari raqam ko sahi se break karega */
    white-space: pre-wrap; /* Spaces aur operators ko sahi line mein rakhega */
    word-break: break-all; /* Kisi bhi point se line break kar dega */
    line-height: 1.4; /* Lines ke darmiyan munasib faasla */
    display: block; /* Poori width istemal karega */
}
...
<div id="custom-popup" class="popup-overlay" onclick="togglePopup()">
    <div class="popup-box" onclick="event.stopPropagation()">
        ...
        <div style="text-align: left; font-size: 0.95rem; line-height: 1.8;">
            ...
            <p><strong>Version:</strong> <span style="color: #bbb;">v2.0.5 (Stable)</span></p>
        </div>
    ...
    <div id="calc-view" class="view active">
        <div class="display-box">
            <input type="text" id="calculation-line" placeholder="0" inputmode="none" style="font-size: 1.4rem; color: #bbb; background: transparent; border: none; text-align: right; width: 100%; outline: none; caret-color: #ff9500;">
...
<script>
    ...
    // 1. Pehle * aur / ko sundar symbols mein badlein
    if (op === '*') op = '×';
    if (op === '/') op = '÷';

    let val = calcLine.value;
    let start = calcLine.selectionStart;
    let lastChar = val.charAt(start - 1);

    // 2. Check mein bhi naye symbols shamil karein
    if (['+', '-', '×', '÷', '*', '/', '%'].includes(lastChar)) {
        calcLine.value = val.substring(0, start - 1) + op + val.substring(start);
        calcLine.setSelectionRange(start, start);
    } else {
        // Baqi normal logic...
        calcLine.value = val.substring(0, start) + op + val.substring(start);
    ...
    const calcLine = document.getElementById('calculation-line');
    let exp = calcLine.value; // Variable ki jagah ab input field se value li hai

    try {
        if (exp) {
            // Math conversion: symbols ko badlein taake computer samajh sakay
            let mathReady = exp.replace(/×/g, '*').replace(/÷/g, '/').replace(/(\d+(\.\d+)?)%/g, "($1/100)");
            
            // Agar aakhir mein symbol (+, -, *, /) nahi hai, to result dikhao
            if (!['+', '-', '*', '/'].includes(mathReady.slice(-1))) {
                let res = eval(mathReady);
                ansLine.innerText = res.toLocaleString(undefined, {maximumFractionDigits: 2});
            }
        } else {
            ansLine.innerText = "0";
        }
    } catch(e) {
        // Calculation adhuri ho to purana answer hi dikhata rahe
    }
}
    ...
    // 1. Agar line khali hai ya aakhir mein pehle se "=" hai, to function rok do
    if (!exp || exp.trim().endsWith('=')) return;

    try {
        // Symbols badlein
        let mathExp = exp.replace(/×/g, '*').replace(/÷/g, '/').replace(/(\d+(\.\d+)?)%/g, "($1/100)");
        
        // Aakhir mein agar operators (+, -, *, /) hon to hata dein
        while(['+', '-', '*', '/'].includes(mathExp.slice(-1))) {
            mathExp = mathExp.slice(0, -1);
            exp = exp.slice(0, -1);
        }

        let res = eval(mathExp);
        let formattedRes = res.toLocaleString(undefined, {maximumFractionDigits: 2});
        ...
        saveToHistory(exp + " =", formattedRes);
        isFinished = true;
        ...
        // Agar koi ghalti ho to "Error" tabhi dikhaye jab waqai raqam ghalat ho
        console.log("Calculation error ignored");
    }
}
